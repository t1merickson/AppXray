...
fileprivate struct Texts {
  ...
  static let wails =
    "A Wails app. Go backend with a native WebView frontend."

  static let nwjs =
    "An NW.js app. Chromium and Node.js under the hood."

  static let gtk =
    "A GTK app. Cross-platform native widgets."

  static let sdl =
    "An SDL app. Cross-platform multimedia and input."
...
extension ExecutableFileTechnologyInfo {
  ...
  private var summaryTextTemplate: String {
    ...
    if features(.wails)       { return Texts.wails       }
    ...
    if features(.gtk) { return Texts.gtk }
    if features(.sdl) { return Texts.sdl }
    ...
    if features(.go)     { return Texts.go     }
    if features(.kotlin) { return Texts.kotlin }
